...
place("target", "cube", (0., 0., 0.), material= "Blade15Back", size_x = 10. * mm, size_y = 10. * mm, size_z = 100. * um)
...
set_output_path(f"Blade15Back_{beam_type}_{str(energy).replace('.','-')}")
set_run_name(f"Blade15Back_{beam_type}_{str(energy).replace('.','-')}_")
config_run(1e8, 12) #Wenn Anwesend dann simulation sonst Interaktiv aber keine Simulation.
start_run(f"./output_runfiles/Blade15Back_{beam_type}_{str(energy).replace('.','-')}.run", force_file_write=True) #Hier Pfad rein und der Code macht eine Datei mit dem Output.
